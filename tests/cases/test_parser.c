...
int test_simple_empty_parsers(Xvr_Bucket** bucket) {
    {
        ...
        if (ast == NULL || ast->type != XVR_AST_END) {
            fprintf(stderr, XVR_CC_ERROR
                    "test_parser(XVR_AST): woilah cik failed to run the "
...
int test_var_declare(Xvr_Bucket** bucketHandle) {
    {
        ...
        if (ast == NULL || ast->type != XVR_AST_BLOCK ||
            ...
            XVR_VALUE_IS_INTEGER(
                ast->block.child->varDeclare.expr->value.value) == false ||
            XVR_VALUE_AS_INTEGER(
                ast->block.child->varDeclare.expr->value.value) != 42) {
            fprintf(stderr,
                    XVR_CC_ERROR
...

    {
        ...
        if (res == 0) {
            printf(XVR_CC_NOTICE
                   "VAR DECLARE: PASSED nice one cik\n" XVR_CC_RESET);
        }
        total += res;
    }
