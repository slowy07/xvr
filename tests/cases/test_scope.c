...
int test_scope_elements() {
  {
    ...
    Xvr_String *hello1 = Xvr_createNameString(&bucket, "hello", XVR_VALUE_NULL);
    Xvr_String *hello2 = Xvr_createNameString(&bucket, "hello", XVR_VALUE_NULL);
...

  {
    ...
    Xvr_String *hello = Xvr_createNameString(&bucket, "hello", XVR_VALUE_NULL);
...

    {
      ...
      if (XVR_VALUE_IS_INTEGER(result1) != true ||
        ...
        fprintf(stderr, XVR_CC_ERROR "ERROR: Failed to access from an ancestor "
                                     "Xvr_Scope\n" XVR_CC_RESET);
...

    {
      ...
      if (XVR_VALUE_IS_FLOAT(result2) != true ||
        ...
        fprintf(stderr, XVR_CC_ERROR
                "ERROR: Failed to shadow an entry in Xvr_Scope\n" XVR_CC_RESET);
        Xvr_freeString(hello);
        while ((scope = Xvr_popScope(scope)) != NULL)
          ;
        Xvr_freeBucket(&bucket);
...

    {
      ...
      if (XVR_VALUE_IS_INTEGER(result3) != true ||
        ...
        fprintf(
            stderr, XVR_CC_ERROR
            "ERROR: Failed to recover an entry in Xvr_Scope\n" XVR_CC_RESET);
        Xvr_freeString(hello);
        while ((scope = Xvr_popScope(scope)) != NULL)
          ;
        Xvr_freeBucket(&bucket);
...

    {
      ...
      if (XVR_VALUE_IS_INTEGER(result4) != true ||
        ...
        fprintf(stderr, XVR_CC_ERROR "ERROR: Failed to assign to an ancestor "
                                     "in Xvr_Scope\n" XVR_CC_RESET);
        Xvr_freeString(hello);
        while ((scope = Xvr_popScope(scope)) != NULL)
          ;
...

    {
      ...
      if (XVR_VALUE_IS_INTEGER(result5) != true ||
        ...
        fprintf(stderr,
                XVR_CC_ERROR "ERROR: Failed to access an altered entry of an "
