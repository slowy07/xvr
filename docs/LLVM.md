...
- Assembly files (ATT or Intel syntax)
...
│   ├── xvr_asm_config.h/.c           # Assembly syntax configuration
...
# Emit assembly with Intel syntax
...
The XVR compiler supports two assembly syntaxes:

| Option | Description |
|--------|-------------|
| `att` | AT&T syntax (default, matches LLVM native output) |
| `intel` | Intel syntax (converted from AT&T) |
...
# Intel syntax (experimental)
xvr script.xvr --emit asm --asm-syntax intel -o program.s
```

**Note:** The Intel syntax option is experimental and may not handle all complex addressing modes correctly. For production use, the default ATT syntax is recommended.
...
```bash
# Emit ATT assembly (default)
$ xvr test.xvr --emit asm -o test.s
$ head -20 test.s
	.file	"test"
...
# Emit Intel syntax (experimental)
$ xvr test.xvr --emit asm --asm-syntax intel -o test.s
$ head -20 test.s
	.file	"test"
	.section	.ltext,"axl",@progbits
	.globl	main
...
**Note:** Intel syntax output is generated by post-processing the ATT output from LLVM. This conversion may not handle all complex addressing modes correctly. For best results, use the default ATT syntax.
