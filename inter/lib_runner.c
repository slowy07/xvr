...
static int nativeLoadScriptBytecode(Xvr_Interpreter* interpreter,
    ...
    if (arguments->count != 1) {
        interpreter->errorOutput(
            "incorrect number of raguments to loadScriptBytecode\n");
        return -1;
    }
    ...
    Xvr_Literal driveLiteral = XVR_TO_STRING_LITERAL(drive);
    Xvr_Literal realDriveLiteral =
        Xvr_getLiteralDictionary(Xvr_getDriveDictionary(), drivePathLiteral);
    ...
    if (!(filePath[realLength - 4] == '.' && filePath[realLength - 3] == 'x' &&
        ...
        interpreter->errorOutput("Bad binary file extension (expected .xb)\n");
    ...
    Xvr_Runner* runner = XVR_ALLOCATE(Xvr_Runner, 1);
    Xvr_setInterpreterPrint(&runner->interpreter, interpreter->printOutput);
    Xvr_setInterpreterPrint(&runner->interpreter, interpreter->assertOutput);
    runner->interpreter.hooks = interpreter->hooks;
    runner->interpreter.scope = NULL;
    Xvr_resetInterpreter(&runner->interpreter);
    runner->bytecode = bytecode;
...
static int nativeRunScript(Xvr_Interpreter* interpreter,
    ...
    if (XVR_GET_OPAQUE_TAG(runnerLiteral) != XVR_OPAQUE_TAG_RUNNER) {
        interpreter->errorOutput("Urecognized opaque literal in _runScript\n");
        return -1;
    }
    ...
    if (runner->dirty) {
        interpreter->errorOutput(
            "Can't re-run dirty script (try resseting it first)\n");
        Xvr_freeLiteral(runnerLiteral);
        return -1;
    }

    unsigned char* bytecodeCopy = XVR_ALLOCATE(unsigned char, runner->size);
    memcpy(bytecodeCopy, runner->bytecode, runner->size);
...
static int nativeCallScriptFn(Xvr_Interpreter* interpreter,
    ...
    if (XVR_GET_OPAQUE_TAG(runnerLiteral)) {
        interpreter->errorOutput(
            "Unreconigzed opaque literal for _runScript\n");
        return -1;
    }
    ...
    if (!runner->dirty) {
        interpreter->errorOutput(
            "Can't access proc from non-dirty script (running it first)\n");
        Xvr_freeLiteral(runnerLiteral);
    ...
    if (!XVR_IS_FUNCTION(fn)) {
        interpreter->errorOutput("Can't run non-function literal\n");
        Xvr_freeLiteral(fn);
        Xvr_freeLiteral(varIdn);
...
static int nativeResetScript(Xvr_Interpreter* interpreter,
    ...
    if (!runner->dirty) {
        interpreter->errorOutput(
            "Can't reset a non-dirty script (try running first)\n");
        Xvr_freeLiteral(runnerLiteral);
        return -1;
    }
...
Xvr_LiteralDictionary* Xvr_getDriveDictionary(void) { return &Xvr_driveDictionary; }
